...
<?php        
        ...
        <img alt="" src= "<?php echo $jiaofu->pic ?>" >
        <h1><?php echo $jiaofu->title; ?>  </h1>
        <p>上场角色：<?php echo $jiaofu->role; ?></p>
        <div class="box"><p>战斗技巧:</p></div>
        <p><?php echo $jiaofu->way; ?></p>
        <?php
         if($jiaofu->av){?>
          <a href="https://www.bilibili.com/video/<?php echo $jiaofu->av ?>">视频</a>
        <?php   }  ?>
        <br/>
  ...
  <form action="save1.php" method="post">
    ...
    <input type="hidden" name='content' value='jiaofu'/>
  ...
  <ol>
  <?php
    $query = $db->query('select * from comments where content= "jiaofu" and post_id = ' . $_GET['id']);
